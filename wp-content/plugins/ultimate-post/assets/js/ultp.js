(function($) {
            ...
            success: function(data) {
                if (data) {
                    ...
                    if (insertPoint.length) {
                        ...
                        if (that.data('blockname').includes("post-module")) {
                            cardDiv.append($('<div style="clear:left;width:100%;padding-block:15px;"></div>'))
                        }
            ...
            input.on('input', function(e) {
                ...
                if (value.length > 0) {
                    dropdown.find('li').each(function() {
                        ...
                        $(this).css('display', currValue.toLowerCase().includes(value) ? 'list-item' : 'none');
                    });
                } else {
                    dropdown.find('li').each(function() {
    ...
    *************************************/
    ...
    function handleTabItem(that) {
        ...
        parent.find('> .ultp-tabs-content-warp > .wp-block-ultimate-post-tab-item').removeClass('pt-active');
        parent.find(`> .ultp-tabs-content-warp > .wp-block-ultimate-post-tab-item.${tabId}`).addClass('pt-active');
    }
    ...
    *************************************/
    ...
    function handleMegaMenuWidth(t) {
        if( $('.editor-styles-wrapper')?.length ) { return }
        const hasMegaMenu = $('.wp-block-ultimate-post-menu-item.hasMegaMenuChild > .ultp-menu-item-wrapper > .ultp-menu-item-content');
        if( hasMegaMenu.length > 0 ) {
            hasMegaMenu.each(function() {
                if( $(this).hasClass('ultpMegaWindowWidth') ) {
                    ...
                    $(this).find(' > .wp-block-ultimate-post-mega-menu > .ultp-mega-menu-wrapper').css({ "maxWidth" : `${bodyWrapperWidth}px`, "boxSizing": "border-box"});
                    // $(this).css({ "left" : `${bodyWrapperLeft - contentLeft}px`});
                    
                    const siblingLeft = $(this).siblings(".ultp-menu-item-label-container")?.offset()?.left || 0;
                    ...
                    const closetMenuWidth = $(this).closest('.wp-block-ultimate-post-menu')?.width() || 800;
                    const closetMenuLeft = $(this).closest('.wp-block-ultimate-post-menu')?.offset()?.left || 0;
                    const contentLeft = $(this)?.offset()?.left || 0;
                    $(this).find(' > .wp-block-ultimate-post-mega-menu > .ultp-mega-menu-wrapper').css({ "maxWidth" : `${closetMenuWidth}px`, "boxSizing": "border-box"});
                    $(this).css({ "left" : `${closetMenuLeft - contentLeft}px`});
                    const siblingLeft = $(this).siblings(".ultp-menu-item-label-container")?.offset()?.left || 0;
                    $(this).css({ "left" : `${closetMenuLeft - siblingLeft}px`});
                } else {
                    $(this).find(' > .wp-block-ultimate-post-mega-menu > .ultp-mega-menu-wrapper').css({ "maxWidth" : ``, "boxSizing": ""});
                    $(this).css({ "left" : ``});
                }
            })
        }
   ...
   if ( isFront ) {
    ...
    if ( isFront ) {
        $('.wp-block-ultimate-post-menu').each( function () {
            ...
            if ( that.data('hasrootmenu') != 'hasRootMenu') {
                that.find(`.ultp-menu-item-wrapper[data-parentbid=".ultp-block-${that?.data('bid')}"] > .ultp-menu-item-label-container a`).each( function () {
                    const theA = $(this);
                    const currentUrl = window.location.href;
                    if ( currentUrl.replace('https:', 'http:') == theA[0].href.replace('https:', 'http:') ) {
                        theA.closest(`.ultp-menu-item-wrapper[data-parentbid=".ultp-block-${that?.data('bid')}"]`).addClass("ultp-current-link");
                    }
                })
            }
        })
        ...
        $(document).on('click', 
            ...
            function(e) {
                if (
                    ...
                    $(e.target).is('.ultp-menu-item-label-container') && $(e.target).siblings('.ultp-menu-item-content').find('> *').length == 0
                ) {
                    return;
                }
                e.preventDefault();
    ...
    function enableDisableMolibeView(e, type='') {      // ham close
        if ( type == 'close' ) {
            $(rawMenu).find("> .ultp-mobile-view-container > .ultp-mobile-view-wrapper").css(
                {
            ...
            setTimeout(()=> {
                ...
                $(rawMenu).find("> .ultp-mobile-view-container").removeClass("ultp-mv-active");
                $(rawMenu).find("> .ultp-mobile-view-container > .ultp-mobile-view-wrapper").css(
                    {
                        "transform": "",
                        "visibility": "",
            ...
            const that = $(e.target);
            mv_animationDuration = $(that).hasClass('ultp-mv-ham-icon') ? $(that).data('animationduration') : that.closest('.ultp-mv-ham-icon').data('animationduration');
            mv_animationDuration = mv_animationDuration || 100;
            mv_HeadText = $(that).hasClass('ultp-mv-ham-icon') ? $(that).data('headtext') : that.closest('.ultp-mv-ham-icon').data('headtext');
            mv_naviIcon = that.closest('.wp-block-ultimate-post-menu').find("> .ultp-mobile-view-container > .ultp-mv-icons > .ultp-mv-label-icon svg").prop('outerHTML');
            mv_naviExpandIcon = that.closest('.wp-block-ultimate-post-menu').find("> .ultp-mobile-view-container > .ultp-mv-icons > .ultp-mv-label-icon-expand svg").prop('outerHTML');
            ...
            menuObj.find("> .ultp-mobile-view-container > .ultp-mobile-view-wrapper").css(
                {
                    "transition-property": "opacity, visibility, transform",
                    "transition-timing-function": "ease-in",
                    "transition-duration": mv_animationDuration ? mv_animationDuration/1000 +"s" : ".25s",
                }
            )
        }
    ...
    function handleResponsiveMenuHtml(e, src) {     // hamIcon next back
        if ( src == 'hamIcon' ) {
            mv_rcsstype = rawMenu.data('rcsstype');
            mv_rstr = rawMenu.data('rstr');
            toAppend =  $(rawMenu).find("> .ultp-mobile-view-container .ultp-mobile-view-body");
            toAppendBack =  $(rawMenu).find("> .ultp-mobile-view-container .ultp-mv-back-label");
            let theHtml = $(rawMenu).find("> .ultp-menu-wrapper > .ultp-menu-content").html();
            
            $(rawMenu).find('.ultp-menu-item-dropdown').toArray().forEach(element => {
                if ( $(element).html() ) {
            ...
            if ( theHtml ) {
                ...
                tempHtml.find('.wp-block-ultimate-post-menu').addClass('ultp-mobile-menu');
                theHtml = tempHtml.html();

                theHtml = handleDropIcon({ type: 'hamicon', _string: theHtml });
                toAppend.html( mv_rcsstype == 'custom' ? theHtml.replaceAll('ultpMenuCss', 'ultpMenu__Css') : theHtml );
            ...
            if ( !prevStack.includes('ultp-block-' + bid) ) {
                ...
                if ( parentItem.hasClass('hasListMenuChild')) {
                    d_none = parentItem.find("> .ultp-menu-item-wrapper > .ultp-menu-item-content > .wp-block-ultimate-post-list-menu").css('display');
                    theHtml = parentItem.find("> .ultp-menu-item-wrapper > .ultp-menu-item-content > .wp-block-ultimate-post-list-menu > .ultp-list-menu-wrapper > .ultp-list-menu-content").html();
                } else {
                    d_none = parentItem.find("> .ultp-menu-item-wrapper > .ultp-menu-item-content > .wp-block-ultimate-post-mega-menu").css('display');
                    theHtml = parentItem.find("> .ultp-menu-item-wrapper > .ultp-menu-item-content").html();
                }
                if ( d_none == 'none' ) { return }
                if ( theHtml ) {
                    $(rawMenu).find('.ultp-mv-back-label-con').removeClass('ultpmenu-dnone');
                    let tempHtml = $('<div>').html(theHtml);
                    tempHtml.find('.wp-block-ultimate-post-menu').addClass('ultp-mobile-menu');
                    theHtml = tempHtml.html();
                    ...
                    toAppendBack.html(parentItem.find("> .ultp-menu-item-wrapper > .ultp-menu-item-label-container .ultp-menu-item-label-text").html());
                    ...
                    if ( mv_rstr == 'mv_dissolve' ) {
                        ...
                        toAppend.html( mv_rcsstype == 'custom' ? theHtml.replaceAll('ultpMenuCss', 'ultpMenu__Css') : theHtml );
                        toAppend.find("> *").css({"opacity": ".1" , transform: "translateX(100%)", transition: `transform ${mv_animationDuration/1000 +"s"} ease` });
                        toAppend.find("> *").animate(
                            {
                                opacity: .3,
                            },
                            10,
                            function() {
                                toAppend.find("> *").css({"opacity": "1" , transform: "translateX(0px)" });
                            }
                        );
                    }
                }
            ...
            let backHtml = '';
            if ( prevStack.length == 0 ) {
                backHtml = $(rawMenu).find("> .ultp-menu-wrapper > .ultp-menu-content").html();
                toAppendBack.html(mv_HeadText);
                $(rawMenu).find('.ultp-mv-back-label-con').addClass('ultpmenu-dnone');
            } else {
                if ( $('.' + currentStack).hasClass('wp-block-ultimate-post-menu-item') ) {
                    if ( $('.' + currentStack).hasClass('hasListMenuChild') ) {
                        backHtml = $(rawMenu).find('.' + currentStack).find("> .ultp-menu-item-wrapper > .ultp-menu-item-content > .wp-block-ultimate-post-list-menu > .ultp-list-menu-wrapper > .ultp-list-menu-content").html();
                    } else {
                        backHtml = $(rawMenu).find('.' + currentStack).find("> .ultp-menu-item-wrapper > .ultp-menu-item-content").html();
                    }
                }
            }
            if ( backHtml ) {
                ...
                toAppendBack.html($(rawMenu).find('.' + currentStack).find("> .ultp-menu-item-wrapper > .ultp-menu-item-label-container .ultp-menu-item-label-text").html());
                ...
                if ( mv_rstr == 'mv_dissolve' ) {
                    toAppend.find("> *").animate(
                        ...
                        mv_animationDuration,
                        function() {
                            toAppend.html( mv_rcsstype == 'custom' ? backHtml.replaceAll('ultpMenuCss', 'ultpMenu__Css') : backHtml );
                        }
                    );
                    toAppend.find("> *").animate(
                        {
    ...
    function handleMenuDropDownStyle(e, type) {
        ...
        if ( c_type == 'next' ) {
            ...
            if ( mv_naviExpandIcon ) {
                parentItem.find("> .ultp-menu-item-wrapper > .ultp-menu-item-label-container .ultp-menu-item-dropdown").html(mv_naviExpandIcon);
            }

            if ( parentItem.hasClass('hasListMenuChild')) {
                target = parentItem.find("> .ultp-menu-item-wrapper > .ultp-menu-item-content > .wp-block-ultimate-post-list-menu > .ultp-list-menu-wrapper > .ultp-list-menu-content");
            } else {
                target = parentItem.find("> .ultp-menu-item-wrapper > .ultp-menu-item-content");
            }
            if ( !target.length ) {
                target = parentItem.find("> .ultp-menu-item-wrapper > .ultp-menu-item-content");
            }
            theHeight = target.outerHeight();
            const paddingObj = parentItem.find("> .ultp-menu-item-wrapper > .ultp-menu-item-content");
            const paddingTop = paddingObj.css("padding-top");
            const paddingBottom = paddingObj.css("padding-bottom");

            parentItem.find("> .ultp-menu-item-wrapper > .ultp-menu-item-content").html(target.html());
            parentItem.find("> .ultp-menu-item-wrapper > .ultp-menu-item-content").css({height: '0px', 'padding-top': '0', 'padding-bottom': '0'});
            parentItem.find("> .ultp-menu-item-wrapper > .ultp-menu-item-content").animate(
                { height: theHeight + 'px', 'padding-top': paddingTop, 'padding-bottom': paddingBottom },
                mv_animationDuration,
                function() {
                    $(this).css({'height': '', 'padding-top': '', 'padding-bottom': ''});
                }
           );
        } else if ( c_type == 'back' ) {
            parentItem.removeClass("ultp-hammenu-accordian-active");
            parentItem.find(".wp-block-ultimate-post-menu-item").removeClass("ultp-hammenu-accordian-active");
            parentItem.find("> .ultp-menu-item-wrapper > .ultp-menu-item-content").animate(
                {
                    height: '0',
                    paddingTop: '0',
                ...
                function() {
                    ...
                    if ( mv_naviIcon ) {
                        parentItem.find(".ultp-menu-item-wrapper .ultp-menu-item-label-container .ultp-menu-item-dropdown").each(function() {
                            if ($(this).html()) {
                    ...
                    parentItem.find(".wp-block-ultimate-post-menu-item").removeClass("ultp-menu-res-css");
                }
           );
        }
    }
    ...
    function HandleRowSticky() {
        if($('.wp-block-ultimate-post-row.row_sticky_active').length > 0) {
            $('.wp-block-ultimate-post-row.row_sticky_active').each(function() {
                ...
                if(rowSelector.hasClass('row_sticky') || rowSelector.hasClass('row_scrollToStickyTop')) {
                    ...
                    $(window).scroll(function(){
                        if(rowSelector.hasClass('row_sticky')) {
                            ...
                            if((windowScroll < $(this).scrollTop()) && (elementPosition  < ($(this).scrollTop() + elementHeight))) {
                                $(rowSelector).animate (
                                    { height: elementHeight - elementHeight }, // Target height
                                    10, 'swing',
                                    function() {  
                                        rowSelector.addClass('stickyTopActive').removeClass('stickyTopDeActive');
                                    }
                                );
                            } else {
                                if((elementPosition + elementHeight) > $(this).scrollTop()) {
                                    rowSelector.addClass('stickyTopDeActive').removeClass('stickyTopActive');
                                }
                                // rowSelector.css({ 'position': 'static !important;'});
                            }
                        } else {
                            if((windowScroll < $(this).scrollTop()) && (elementHeight < $(this).scrollTop())) {
                                rowSelector.addClass('stickyTopActive').removeClass('stickyTopDeActive');
                            } else {
                                rowSelector.addClass('stickyTopDeActive').removeClass('stickyTopActive');
                            }
